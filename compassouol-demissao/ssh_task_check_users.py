...
if args.deluser == None:
    ...
    del_ldap_user = (f'sudo /bin/disableuser '+ tasknumber +' '+ delusername +'\n')
    del_puppet_user = (f'sudo /bin/disableuser '+ tasknumber +' '+ delusername +'\n')
    del_tacacs_user = (f'sudo /bin/disableuser '+ tasknumber +' '+ delusername +'\n')
...
def main():
    if args.deluser == None:
        ...
        for device in conteudo_json:
           ...
           elif device["ip"] == "187.16.26.114":
                result = ssh_send_cmd(device, check_users_passwd)
                result = ssh_send_cmd(device, check_users_puppet)
           elif device["ip"] == "192.168.248.172":
                result = ssh_send_cmd(device, check_users_passwd)
                result = ssh_send_cmd(device, check_users_ldap)
           elif device["ip"] == "10.11.8.3":
                result = connect_jump(device, check_users_tacplus)
        ...
        for device in conteudo_json:
           if device["ip"] == "187.16.26.35":
                ...
                eventlog(device_IP, tasknumber)
                result = ssh_send_cmd(device, del_user)
                result = ssh_send_cmd(device, del_puppet_user)
           elif device["ip"] == "192.168.248.172":
                device_IP = device['ip']
                eventlog(device_IP, tasknumber)
                result = ssh_send_cmd(device, del_user)
                result = ssh_send_cmd(device, del_ldap_user)
           elif device["ip"] == "10.11.8.3":
                device_IP = device['ip']
                eventlog(device_IP, tasknumber)
                result = connect_jump(device, del_user)
                result = connect_jump(device, del_tacacs_user)
        print(f"\n{'#' * 50}\nFinished Executing Script\n{'#' * 50}")
        sys.exit()
